...
- [x] 桌面视口未出现页面级横向溢出。
- [x] 生产版确定性配置下，1% 精确比例优化完整评估 5,151 个方案并返回结果。
...
- 优化器坚持 exact search；1% 比例有 5,151 个候选，24 名 supported 车身选四且允许重复有 17,550 组，联合理论空间为 90,400,050。
- 含多个概率技能的默认真实阵容会显著增加每个候选的精确状态传播成本。优化已放入 Web Worker，不会冻结页面主线程，但完成时间可能达到数分钟。
- 0.01% 比例全网格理论上有 50,015,001 个候选；当前浏览器版不适合直接运行该完整空间。
