...
审计口径：2026-09-10 v0.3 最新确认规则。数据仅来自 `src/game-data` 并经 `collectSkillCatalog()` 汇总。`pending` 不进入正式伤害或优化评分；`DATA_SOURCE_UNCERTAIN` 占位英雄在正常 UI 隐藏。
...
- 韦恩：round5/9 extraDamage；三兵种普通攻击各自独立25%暴击、倍率2、只作用普通攻击。
- 琳恩：每回合40%概率穿透；射手第3次普通攻击后获得首层攻击，round4首次生效。
- 修拉/丽姬娅：射手普通攻击计数回合已确认；丽姬娅两技能显式共享计数器。
- 乌尔卡努丝：复用格温第6次额伤与第8次易伤覆盖语义；三技能round3/6/9生效。
