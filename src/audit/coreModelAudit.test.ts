...
describe("Stage 24 stable core audit", () => {
  ...
  it("真实技能统计稳定且剩余阻塞严格区分RULE_UNKNOWN与DATA_SOURCE_UNCERTAIN", () => {
    ...
    expect(collectSkillCatalog()).toHaveLength(59);
    expect(support.totals.skills).toEqual({ supported: 55, pending: 4, unsupported: 0 });
    expect(support.totals.effects).toEqual({ supported: 56, pending: 0, unsupported: 0 });
