...
import {
  ...
  TROOP_LABELS, TROOP_TYPES, type CalculatorFormState,
  type UiSelectOption,
...
const TOWN_KEYS = ["attack", "penetration", "defenseReduction", "marchCapacity"] as const;
const PET_KEYS = ["attackLevel", "penetrationLevel", "defenseReductionLevel", "capacityLevel"] as const;
...
export function CalculatorApp() {
  ...
  const updateTroop = (type: typeof TROOP_TYPES[number], key: "count" | "troopLevelId" | "attackPercent" | "defensePercent" | "penetrationPercent" | "healthPercent", value: string) =>
    setForm((current) => ({ ...current, troops: { ...current.troops, [type]: { ...current.troops[type], [key]: value } } }));
  ...
  const calculate = () => {
    ...
    try { setResult(calculateUiDamage(form)); } catch (caught) { setError(errorText(caught)); } finally { setBusy(null); }
  };
  ...
  return <div className="app-shell">
    ...
    <main>
      <section className="panel inputs">
        <div className="section-title"><div><h2>战报与阵容</h2><p className="section-help">打熊阵容无宠无药打野怪数据</p></div><span className="total">原始阵容兵数 {nf.format(total)}</span></div>
        <div className="troop-grid">{TROOP_TYPES.map((type) => <fieldset key={type}><legend>{TROOP_LABELS[type]}</legend>
          <label>兵数<input aria-label={`${TROOP_LABELS[type]}兵数`} type="number" min="0" value={form.troops[type].count} onChange={(event) => updateTroop(type, "count", event.target.value)} /></label>
          <label>等级<select value={form.troops[type].troopLevelId} onChange={(event) => updateTroop(type, "troopLevelId", event.target.value)}>{knownTroopLevelOptions.map((level) => <option key={level.id}>{level.id}</option>)}</select></label>
          <label>攻击加成 %<input aria-label={`${TROOP_LABELS[type]}攻击加成`} type="number" value={form.troops[type].attackPercent} onChange={(event) => updateTroop(type, "attackPercent", event.target.value)} /></label>
          <label>防御加成 %<input type="number" value={form.troops[type].defensePercent} onChange={(event) => updateTroop(type, "defensePercent", event.target.value)} /></label>
          <label>穿透加成 %<input type="number" value={form.troops[type].penetrationPercent} onChange={(event) => updateTroop(type, "penetrationPercent", event.target.value)} /></label>
          <label>生命加成 %<input type="number" value={form.troops[type].healthPercent} onChange={(event) => updateTroop(type, "healthPercent", event.target.value)} /></label>
        </fieldset>)}</div>

        <h3>出征容量与专家</h3><div className="two-col">
          <div className="capacity-summary"><span>基础阵容兵数</span><strong>{nf.format(total)}</strong><small>由盾兵、矛兵、射手原始输入自动求和</small></div>
          ...
          <label>巨熊克星<LevelSelect value={form.preparation.bearSlayerLevel} options={bearSlayerLevelOptions} onChange={(value) => updatePreparation("bearSlayerLevel", value)} /></label>
        </div>
        <h3>城镇增益</h3><div className="two-col">{TOWN_KEYS.map((key) => <label key={key}>{townLabels[key]}<select value={form.preparation.town[key]} onChange={(event) => setForm((current) => ({ ...current, preparation: { ...current.preparation, town: { ...current.preparation.town, [key]: event.target.value as "none" | "small" | "large" } } }))}><option value="none">无</option><option value="small">小药 10%</option><option value="large">大药 20%</option></select></label>)}</div>
        <h3>宠物增益</h3><div className="two-col">{PET_KEYS.map((key) => <label key={key}>{petLabels[key]}<LevelSelect value={form.preparation.pet[key]} options={key === "capacityLevel" ? petCapacityLevelOptions : petBuffLevelOptions} onChange={(value) => setForm((current) => ({ ...current, preparation: { ...current.preparation, pet: { ...current.preparation.pet, [key]: value } } }))} /></label>)}</div>
